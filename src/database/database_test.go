	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			case false:
				...
				db, _, _ := sqlmock.NewWithDSN(database.DSN(database.PostgreSQLInfo{})) //.Config.Database.PostgreSQL))
				database.DB, _ = database.SetupPostgres(database.Info{})
				if tc.want != (database.DB == db) {
					t.Error("Error in test: ", tc.name)
				}
			case true:
				database.DB, _ = database.SetupPostgres(database.Info{})
				db, err := database.SetupPostgres(database.Info{})
				if err != nil {
